	...
	for {
		...
		case t := <-c.taskCh:
			if t.phase == mapPhase {
				...
				path := mergeName(t.dataDir, t.jobName, t.taskNumber)
				a,b:=CreateFileAndBuf(path)
				b.Buffered()			
				if err := ioutil.WriteFile(path, []byte(strings.Join(res, "")), 0600); err != nil {
					panic(err)
				}
				// YOUR CODE HERE :)
...
func (c *MRCluster) run(jobName, dataDir string, mapF MapF, reduceF ReduceF, mapFiles []string, nReduce int, notify chan<- []string) {
	...
	for i := 0; i < nReduce; i++ {
		...
		t.wg.Add(1)
		rdtasks = append(rdtasks, t)
		go func() { c.taskCh <- t }()
		Path[t.taskNumber] = mergeName(t.dataDir,t.jobName,t.taskNumber)
	}
	for _, t := range rdtasks {
		t.wg.Wait()
	}
	notify <- Path
}
